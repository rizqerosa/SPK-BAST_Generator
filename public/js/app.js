...
function autoGenerateDocNumbers(inputTahunVal) {
  ...
  spkRecords.forEach(r => {
    if (parseInt(r.Tahun) === yr && r.Nomor_SPK) {
      const matches = String(r.Nomor_SPK).match(/\d+/g);
      if (matches) {
  ...
  allBastRecords.forEach(r => {
    if (parseInt(r.Tahun) === yr) {
      const bastFields = [r.No_BAST_PPL_PML, r.No_BAST_PPL_SM, r.No_BAST_PML_SM, r.No_BAST_SM_PPK];
