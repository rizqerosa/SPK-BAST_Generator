...
function autoGenerateDocNumbers(inputTahunVal) {
  ...
  const inputNoSpk = document.getElementById("input-no-spk");
  if (inputNoSpk) {
    inputNoSpk.value = `${nextSpkSeq} Tahun ${yr}`;
  }
...
async function handleFormSubmit(e) {
  ...
  const record = {
    ...
    Judul_Pekerjaan_Dokumen: (typeof buildJudulDenganPeriode === "function" ? buildJudulDenganPeriode(fd.get("judul_pekerjaan") || "", {
      Bulan: fd.get("bulan"),
      Tahun: fd.get("tahun"),
      Tanggal_Mulai: fd.get("tanggal_mulai"),
      Tanggal_SPK: fd.get("tanggal_spk")
    }, details) : (fd.get("judul_pekerjaan") || "")),
    Tanggal_Mulai:           fd.get("tanggal_mulai") || "",
...
async function renderTab(tabKey, ctx, containerId, record, details) {
  ...
  if (tabKey === "sm-ppk") {
    ...
    if (!bastRecord && record) {
      const fullT = typeof buildJudulDenganPeriode === "function" ? buildJudulDenganPeriode(record.Judul_Pekerjaan_Dokumen, record) : record.Judul_Pekerjaan_Dokumen;
      bastRecord = {
        Judul_Pekerjaan_Dokumen: fullT,
